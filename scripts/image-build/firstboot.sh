...
def main():
    ...
    try:
        # Drop the stale invalid quirks file from older builds, if present.
        if os.path.exists(STALE_QUIRK):
            try:
                with open(STALE_QUIRK) as f:
                    stale = "FoodAssistant touchscreen calibration" in f.read()
                if stale:
    ...
    print("Wrote %s. Restart the kiosk (or reboot) to apply." % RULES)
    return 0
